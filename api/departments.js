...
router.route("/:id/departments")
  .post(authenticateUser, requireBody(["name", "description", "banner_image"]), async (req, res) => {
    try {
      const { id } = req.params;
      const { name, description, banner_image } = req.body;
      const department = await createDepartment(name, description, banner_image, id);
...
router.route("/:id/departments").delete(async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) { // ← Fixed: rows not row
...
router.route("/:id")
  .put(authenticateUser, requireBody(["name", "description", "banner_image"]), async (req, res) => {
    try {
      ...
      if (req.user.role !== 'admin' && req.user.id !== existingDepartment.userId) {
        return res.status(403).json({ error: "Forbidden: Cannot update this department" });
      }
      
      const updatedDepartment = await updateDepartment(departmentId, { name, description, banner_image });
